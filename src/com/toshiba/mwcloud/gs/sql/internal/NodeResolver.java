...
class NodeResolver implements Closeable {
	...
	private boolean updateConnectionAndClusterInfo(ClusterInfo clusterInfo)
		...
		try {
			...
			final boolean masterResolvable =
					isMasterResolvableByConnection();
			...
			if (masterResolvable) {
				...
				masterMatched = true;
				hashMode = getDefaultContainerHashMode();
			}
	...
	private static boolean isMasterResolvableByConnection() {
		return NodeConnection.getProtocolVersion() >= 8;
	}

	private static ContainerHashMode getDefaultContainerHashMode() {
